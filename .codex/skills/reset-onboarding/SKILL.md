...
全程用 `optima-query-db`，`<env>` 取 `cn-stage` / `cn-prod`，作为第 3 个位置参数（如 `optima-query-db billing "..." cn-stage`）。先加载 cn 凭证：

```bash
source ~/.infisical_cn_creds && export OPTIMA_CN_BUILDBOX_PASSWORD="$(cat ~/.buildbox_pw)"
```

**① 标识符 → uid**（手机号/邮箱查 user-auth；已有 uid 跳过）。注意 cn-prod 与 cn-stage 是独立库，同一手机号两边 uid 不同：
...
> 回显里同时确认 `granted > 0`——若 `granted=0`，UPDATE 照跑也是无效重置（公式 `(granted-used)>0` 不成立）。若查不到行或 `granted=0`：该账号从未被授予引导资格，不属「重置」范畴——发放 grant 走 billing 侧逻辑，找 billing owner 确认。

**③ 重置——`used` 和 `spent_credits` 两个字段都要清、缺一不可**（只清 `used`、若 `spent_credits` 已烧到 cap 则资格仍是 false）：

```bash
optima-query-db billing "UPDATE guided_run_grant SET used=0, spent_credits=0, updated_at=now() WHERE user_id='<uid>' RETURNING user_id, granted, used, cap_credits, spent_credits" <env>
```
...
1. `cn-prod` 是生产库写操作：执行 UPDATE 前必须先跑 ②，核对回显的 uid/手机号确系目标账号。
