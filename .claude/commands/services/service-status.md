# /service-status - 查看服务状态

查看所有服务的运行状态，以表格形式展示。

## 使用场景

**所有开发者**: 快速了解当前环境所有服务的运行状态
**DevOps**: 监控服务健康状态，发现异常服务
**调试**: 排查服务间依赖问题

## 用法

/service-status [environment]

## 参数

- `environment` (可选): 环境
  - `local` - 本地 Docker 环境（默认）
  - `stage` - Stage-ECS 环境
  - `prod` - 生产环境
  - `all` - 所有环境对比

## 执行逻辑

1. 识别目标环境
2. 并发检查所有服务状态
3. 收集关键指标（运行状态、端口、健康检查、资源使用）
4. 以表格形式展示结果
5. 高亮异常服务

## 命令示例

### 本地环境 - Docker Compose

```bash
# 检查 Docker 容器状态
docker compose ps --format json

# 检查服务健康
curl -s -o /dev/null -w "%{http_code}" http://localhost:8280/health  # commerce-backend
curl -s -o /dev/null -w "%{http_code}" http://localhost:8290/health  # user-auth
curl -s -o /dev/null -w "%{http_code}" http://localhost:8300/health  # mcp-host

# 检查数据库
pg_isready -h localhost -p 8282 -U commerce_user

# 检查 Redis
redis-cli -h localhost -p 8285 ping

# 检查 MinIO
curl -s -o /dev/null -w "%{http_code}" http://localhost:8284/minio/health/live
```

### Stage-ECS

```bash
# 获取 ECS 服务状态
aws ecs describe-services \
  --cluster optima-stage \
  --services commerce-backend-stage user-auth-stage mcp-host-stage

# 检查健康端点
curl -s -o /dev/null -w "%{http_code}" https://api.stage.optima.onl/health
curl -s -o /dev/null -w "%{http_code}" https://auth.stage.optima.onl/health
curl -s -o /dev/null -w "%{http_code}" https://mcp.stage.optima.onl/health

# 检查 RDS
aws rds describe-db-instances --db-instance-identifier optima-stage-postgres
```

### Prod

```bash
# 通过 SSH 检查 Docker 容器
ssh -i ~/.ssh/optima-ec2-key ec2-user@ec2-prod.optima.shop \
  "docker ps --format '{{.Names}}\t{{.Status}}'"

# 检查健康端点
curl -s -o /dev/null -w "%{http_code}" https://api.optima.shop/health
curl -s -o /dev/null -w "%{http_code}" https://auth.optima.shop/health
curl -s -o /dev/null -w "%{http_code}" https://mcp.optima.shop/health

# 检查 RDS
aws rds describe-db-instances --db-instance-identifier optima-prod-postgres
```

## 预期输出

### 本地环境（全部正常）

```
📊 服务状态报告 - 本地环境 (local)

┌─────────────────────┬──────────┬────────┬────────────┬─────────────┬──────────┐
│ 服务                │ 状态     │ 端口   │ 健康检查   │ 启动时间    │ CPU/内存 │
├─────────────────────┼──────────┼────────┼────────────┼─────────────┼──────────┤
│ commerce-backend    │ ✅ 运行  │ 8280   │ ✅ 200 OK  │ 2小时前     │ 5%/120MB │
│ user-auth           │ ✅ 运行  │ 8290   │ ✅ 200 OK  │ 2小时前     │ 3%/80MB  │
│ mcp-host            │ ✅ 运行  │ 8300   │ ✅ 200 OK  │ 2小时前     │ 4%/100MB │
│ postgres            │ ✅ 运行  │ 8282   │ ✅ 就绪    │ 3小时前     │ 2%/150MB │
│ redis               │ ✅ 运行  │ 8285   │ ✅ PONG    │ 3小时前     │ 1%/30MB  │
│ minio               │ ✅ 运行  │ 8283/4 │ ✅ 200 OK  │ 3小时前     │ 1%/50MB  │
└─────────────────────┴──────────┴────────┴────────────┴─────────────┴──────────┘

📊 总览:
✅ 运行中: 6/6
⚠️ 警告: 0
❌ 异常: 0

🌐 访问地址:
• Commerce Backend: http://localhost:8280/docs
• User Auth: http://localhost:8290/docs
• MCP Host: http://localhost:8300/docs
• MinIO Console: http://localhost:8284
```

### 本地环境（部分异常）

```
📊 服务状态报告 - 本地环境 (local)

┌─────────────────────┬──────────┬────────┬────────────┬─────────────┬──────────┐
│ 服务                │ 状态     │ 端口   │ 健康检查   │ 启动时间    │ CPU/内存 │
├─────────────────────┼──────────┼────────┼────────────┼─────────────┼──────────┤
│ commerce-backend    │ ✅ 运行  │ 8280   │ ✅ 200 OK  │ 10分钟前    │ 5%/120MB │
│ user-auth           │ ❌ 停止  │ -      │ ❌ 不可用  │ -           │ -        │
│ mcp-host            │ ⚠️ 不健康│ 8300   │ ❌ 503     │ 1小时前     │ 45%/500MB│
│ postgres            │ ✅ 运行  │ 8282   │ ✅ 就绪    │ 2小时前     │ 2%/150MB │
│ redis               │ ✅ 运行  │ 8285   │ ✅ PONG    │ 2小时前     │ 1%/30MB  │
│ minio               │ ✅ 运行  │ 8283/4 │ ✅ 200 OK  │ 2小时前     │ 1%/50MB  │
└─────────────────────┴──────────┴────────┴────────────┴─────────────┴──────────┘

📊 总览:
✅ 运行中: 4/6
⚠️ 警告: 1/6 (mcp-host)
❌ 异常: 1/6 (user-auth)

⚠️ 需要处理的问题:

1. ❌ user-auth 已停止
   原因: 容器未运行
   建议: /restart-service user-auth

2. ⚠️ mcp-host 不健康 (HTTP 503)
   原因: 服务启动但健康检查失败，内存使用过高 (500MB)
   建议:
   - 查看日志: /backend-logs mcp-host
   - 重启服务: /restart-service mcp-host
   - 检查 MCP 服务器连接
```

### Stage-ECS 环境

```
📊 服务状态报告 - Stage-ECS (stage)

┌─────────────────────────┬──────────┬──────────────┬────────────┬─────────────┐
│ 服务                    │ 状态     │ 任务数       │ 健康检查   │ 最后部署    │
├─────────────────────────┼──────────┼──────────────┼────────────┼─────────────┤
│ commerce-backend-stage  │ ✅ 运行  │ 2/2 (稳定)   │ ✅ 200 OK  │ 3天前       │
│ user-auth-stage         │ ✅ 运行  │ 1/1 (稳定)   │ ✅ 200 OK  │ 5天前       │
│ mcp-host-stage          │ ✅ 运行  │ 1/1 (稳定)   │ ✅ 200 OK  │ 2天前       │
│ optima-stage-postgres   │ ✅ 运行  │ -            │ ✅ 可用    │ -           │
│ optima-stage-redis      │ ✅ 运行  │ -            │ ✅ 可用    │ -           │
└─────────────────────────┴──────────┴──────────────┴────────────┴─────────────┘

📊 总览: 5/5 服务正常

🌐 访问地址:
• Commerce Backend: https://api.stage.optima.onl
• User Auth: https://auth.stage.optima.onl
• MCP Host: https://mcp.stage.optima.onl
```

### 所有环境对比

```
📊 服务状态报告 - 所有环境对比

┌───────────────────┬──────────────┬──────────────┬──────────────┐
│ 服务              │ Local        │ Stage        │ Prod         │
├───────────────────┼──────────────┼──────────────┼──────────────┤
│ commerce-backend  │ ✅ 运行      │ ✅ 运行      │ ✅ 运行      │
│ user-auth         │ ❌ 停止      │ ✅ 运行      │ ✅ 运行      │
│ mcp-host          │ ✅ 运行      │ ✅ 运行      │ ✅ 运行      │
│ postgres          │ ✅ 运行      │ ✅ 运行      │ ✅ 运行      │
│ redis             │ ✅ 运行      │ ✅ 运行      │ ✅ 运行      │
└───────────────────┴──────────────┴──────────────┴──────────────┘

💡 环境差异:
- Local 的 user-auth 已停止，需要重启
```

## 扩展检查项

### 包含前端应用

```bash
# 检查 optima-store (前端)
curl -s -o /dev/null -w "%{http_code}" http://localhost:3001

# 检查 agentic-chat (前端)
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000
```

### 包含 MCP 工具服务

```bash
# 检查 Commerce MCP
curl -s -o /dev/null -w "%{http_code}" http://localhost:8230/health

# 检查 Scout MCP
curl -s -o /dev/null -w "%{http_code}" http://localhost:8250/health

# 检查 Comfy MCP
curl -s -o /dev/null -w "%{http_code}" http://localhost:8220/health

# 检查 Google Ads MCP
curl -s -o /dev/null -w "%{http_code}" http://localhost:8240/health
```

## 故障排查

### Docker 容器未运行

```
Error: No such container
```

- 启动 Docker Compose: `docker compose up -d`
- 检查 docker-compose.yml 配置

### AWS 凭证问题

```
Error: Unable to locate credentials
```

- 配置 AWS CLI: `aws configure`
- 或设置环境变量: `AWS_PROFILE=optima`

### SSH 连接失败

```
Error: Permission denied
```

- 检查 SSH 密钥: `chmod 600 ~/.ssh/optima-ec2-key`
- 验证 EC2 主机地址

## 相关命令

- /health-check - 详细健康检查
- /backend-logs - 查看服务日志
- /restart-service - 重启异常服务
